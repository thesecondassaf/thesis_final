...
\begin{proof}
    ...
    \textcolor{red}{We claim that it follows that $\cP^{\deg \ndls} \to U^{(\deg ndls)} $ is totally ramified at $\eta_{\ndls}$ as well.}
    Thus, take a subgroup $H \subset G$ such that $G/H \cong \Z/p \Z$ and decompose $\cP \to U$ as:
    $\cP \to \cP_{G/H} \to U$ as in \Cref{{prop:quotient_torsors}}.
    Then $\cP_{G/H} \to U$ is a $G/H \cong \Z/p \Z$ torsor so $\cP_{G/H}^{(\deg \ndls)}$ is unramified at $\eta_{\ndls}$,
    which implies $\cP^{(\deg \ndls)}$ is unramified at $\eta_\ndls$ which implies $\cP^{(\deg \ndls)}$ acts trivially.     
\end{proof}
...
\input{sections/Corrolaries.tex}