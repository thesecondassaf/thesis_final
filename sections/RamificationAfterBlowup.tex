...
\end{cor}
\begin{proof} The first part that $\boxP{\cP^{(\deg \mdls)}}{\cP^{(d - \deg \mdls)}}$ is tamely ramified at $\theta$ is our discussion above.
The fact that $\cP^{(d)}$ is tamely ramified at $\eta_0$ will follow from \Cref{lemma:descend_ramification_along_etale}, once we 
show the conditions there are met:



% follows from \Cref{cor:etale_at_generic_point} and \Cref{section:symmetric_powers_on_curves}
% (And the fact that an etale map restricts to unramified extension of the residue fields).
\end{proof}
