...
We review some fundamental results concerning group actions on schemes, the theory of torsors, 
and their respective quotients. 
In general, the categorical quotient of a scheme by a group action is an algebraic space or a stack and does not necessarily admit a scheme structure. 
However, under specific conditions—such as those present in our current setting—the quotient exists as a scheme. 
Given that this material is thoroughly organized in \cite{Guignard2018}, based on \cite{sga1} our exposition follows their exposition closely.
We omit some details though.
...
\begin{prop}[\cite{Guignard2018} Proposition 2.27]
    ...
    Consequently, the quotient $\mathrm{Sym}_S^d(X) = X^{\times_S d}/S_d$ exists as a scheme over $S$. When the base $S$ is understood from context, this quotient is also denoted by $X^{(d)}$.
\end{prop}
