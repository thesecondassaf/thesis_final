...
\textcolor{red}{where is the first tiem you introduce $\cP$ instead of $P$, mark it down, and explain}
...
We recall some algebraic preliminaries on ramification theory of discrete valuation rings, following \stackstag{0EXQ}.
...
Note that in the case of global and local fields (taking $A$ to be the local ring at a prime of the ring of integers), this definition of tame ramification coincides with the usual one.
...
By \Cref{prop:torsor_representable}, $\cP$ is representable by a finite etale $U$-scheme. The conditions of \Cref{definition:tamely_ramified_in_codim_1} are satisfied.
And thus we can speak about $\cP$ being tamely ramified (or unramified) over $C$.

Moreover, %expand on the situation here, explain why the fields invloved are galois and we can define higher ramification groups.% 

%The fields $L_\xi$ 

\begin{definition}
A $G$-torsor $\cP$ on $U_{\text{ét}}$ has \textbf{ramification bounded by} $\mdls$ \textbf{over} $\Spec(k)$ if for every index $i$ and every geometric point 
$\bar{x} \in C(\bar{k})$ lying over $P_i$, the restriction of $\cP$ to 
\[
\Spec(\widehat{\mathcal{O}_{C_{\bar{k}},\bar{x}}}) \times_{C_{\bar{k}}} U_{\bar{k}}
\]
has ramification bounded by the multiplicity $n_i$ (i.e., the multiplicity of $\mathfrak{m}$ at $P_i$).
\end{definition}
