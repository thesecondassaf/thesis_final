...
The next lemma shows that ramification doesn't change under etale maps. 
\begin{lemma}\label{lemma:descend_ramification_along_etale}
    Let $X, U, f: Y \to U$ be as in \Cref{definition:tamely_ramified_in_codim_1}.
    And let $(Z, \xi)$ be a prime divisor on $X$ with generic point $\xi$ such that $Z \cap U = \emptyset$.
    Assume there is a morphism $g: X' \to X$, such that $U' = X' \times_X U$ is dense open subset, $(Z' = X' \times_X Z, \xi' = X' \times_X \xi)$
    is a prime divisor with generic point $\xi'$.
    \textbf{If $g$ is etale at $\xi'$}, then $f$ is unramified (resp. tamely ramified) at $(Z, \xi)$ if and only if 
    $g': Y'=X' \times_X Y \to U'$ is unramified (resp. tamely ramified) at $(Z', \xi')$.
\end{lemma}
\begin{proof}
Let $\mathcal{O}_{X,\xi}$ and $\mathcal{O}_{X',\xi'}$ be the discrete valuation rings (DVRs) associated with the generic points of $Z$ and $Z'$, and let $\mathfrak{m}_{\xi}$ and $\mathfrak{m}_{\xi'}$ be their respective maximal ideals. Let $\eta \in Y$ be a point such that $f(\eta) = \xi$, and let $\eta' \in Y'$ be the corresponding point in the fiber product $Y' = X' \times_X Y$ such that $g'(\eta') = \eta$ and $f'(\eta') = \xi'$.

\medskip
\noindent \textbf{1. Equality of Ramification Indices:} \\
By definition, the ramification index $e(f, \eta)$ is the unique integer $e$ such that $\mathfrak{m}_{\xi} \mathcal{O}_{Y,\eta} = \mathfrak{m}_{\eta}^e$. Since $g: X' \to X$ is \textbf{étale} at $\xi'$, the map of local rings $\mathcal{O}_{X,\xi} \to \mathcal{O}_{X',\xi'}$ is an étale local homomorphism. This implies:
\begin{itemize}
    \item $g$ is unramified at $\xi'$, so $\mathfrak{m}_{\xi} \mathcal{O}_{X',\xi'} = \mathfrak{m}_{\xi'}$.
    \item The morphism $f'$ is obtained by the base change $Y' = Y \times_X X'$. Localizing at $\eta'$, we have $\mathcal{O}_{Y',\eta'} \cong \mathcal{O}_{Y,\eta} \otimes_{\mathcal{O}_{X,\xi}} \mathcal{O}_{X',\xi'}$.
\end{itemize}
Now, calculate the ramification index $e(f', \eta')$:
\[
\mathfrak{m}_{\xi'} \mathcal{O}_{Y',\eta'} = (\mathfrak{m}_{\xi} \mathcal{O}_{X',\xi'}) \mathcal{O}_{Y',\eta'} = \mathfrak{m}_{\xi} (\mathcal{O}_{Y,\eta} \otimes_{\mathcal{O}_{X,\xi}} \mathcal{O}_{X',\xi'}) = (\mathfrak{m}_{\xi} \mathcal{O}_{Y,\eta}) \otimes_{\mathcal{O}_{X,\xi}} \mathcal{O}_{X',\xi'}
\]
Substituting $\mathfrak{m}_{\xi} \mathcal{O}_{Y,\eta} = \mathfrak{m}_{\eta}^e$, we get:
\[
\mathfrak{m}_{\xi'} \mathcal{O}_{Y',\eta'} = \mathfrak{m}_{\eta}^e \otimes_{\mathcal{O}_{X,\xi}} \mathcal{O}_{X',\xi'} = (\mathfrak{m}_{\eta} \otimes_{\mathcal{O}_{X,\xi}} \mathcal{O}_{X',\xi'})^e = \mathfrak{m}_{\eta'}^e
\]
Thus, $e(f', \eta') = e(f, \eta)$.

\medskip
\noindent \textbf{2. Residue Field Extensions:} \\
Let $\kappa(\xi)$ and $\kappa(\eta)$ denote the residue fields. The property of being unramified or tamely ramified depends on the extension $\kappa(\eta) / \kappa(\xi)$. Under the étale base change $g$, the new residue field extension is:
\[
\kappa(\eta') = \kappa(\eta) \otimes_{\kappa(\xi)} \kappa(\xi')
\]
Because $\kappa(\xi') / \kappa(\xi)$ is a finite separable extension (as $g$ is étale), properties of the extension $\kappa(\eta) / \kappa(\xi)$ are preserved:
\begin{itemize}
    \item $\kappa(\eta) / \kappa(\xi)$ is separable $\iff$ $\kappa(\eta') / \kappa(\xi')$ is separable.
    \item The characteristic $p$ of the residue field is preserved. Since $e = e'$, then $p \nmid e \iff p \nmid e'$.
\end{itemize}

\medskip
\noindent \textbf{Conclusion:} \\
Since $e = e'$ and the separability of the residue field extension is invariant under étale base change, $f$ is unramified (resp. tamely ramified) at $(Z, \xi)$ if and only if $f'$ is unramified (resp. tamely ramified) at $(Z', \xi')$.
\end{proof}

\begin{rem*}
For a Galois cover $f: Y \to U$, the equivalence established in the lemma extends naturally to the study of higher ramification groups. Specifically, under the same assumptions for the morphism $g: X' \to X$, the lemma remains valid if the conditions of being unramified or tamely ramified are replaced by the property of having ramification bounded by $r$.\end{rem*}
