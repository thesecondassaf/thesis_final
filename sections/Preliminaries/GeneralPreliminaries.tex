...
We record this equivalence once below. Thereafter, the main results are stated in
the language of local systems, while the geometric and ramification calculations
are carried out with the corresponding torsors.
...
We now record the equivalence used in the thesis.
...
\[
\mathbf{Tors}(\mathcal{E},G)
\xrightarrow{\ \sim\ }
\mathbf{Pic}(\mathcal{E},\Lambda).
\]
\end{prop}
...
\operatorname{Fr}(\mathcal{P}\times^G\Lambda),
\qquad
p\longmapsto\bigl(\lambda\longmapsto[p,\lambda]\bigr),
\]
and
\[
\operatorname{Fr}(\mathcal{L})\times^G\Lambda
\longrightarrow\mathcal{L},
\qquad
[\varphi,\lambda]\longmapsto\varphi(\lambda).
\]
After passing to a cover on which $\mathcal{P}$ is the trivial torsor and
$\mathcal{L}$ is free of rank one, both morphisms become the identity.
...
$\Lambda$-local systems on $X$. Henceforth, we identify such a local system
with its $G=\Lambda^\times$-torsor of frames and pass between the two
descriptions without further comment.\footnote{The equivalence is compatible
with pullback and with tensor products, hence with the symmetric-power
constructions used below. These compatibilities may be checked after a local
trivialization. It also preserves ramification bounds: a rank-one local
system and its frame torsor determine, on each connected component, the same character
$\pi_1^{\mathrm{et}}(X,\bar{x})\to\Lambda^\times$, and therefore have the
same restrictions to inertia and to the upper ramification groups.}
...
% [PROPOSED NOTE, for review]
{\color{green!50!black}
\subsection*{Torsors and first cohomology}
\phantomsection\label{section:torsors_and_cohomology_note}
We use the following standard fact without proof
\cite[III, \S4]{milneEC}. Let $G$ be an abelian sheaf of groups on
$X_{\mathrm{\acute et}}$. Then there is a natural group isomorphism
\[
\Tors(X_{\mathrm{\acute et}},G)\xrightarrow{\ \cong\ }
H^1_{\mathrm{\acute et}}(X,G),
\]
and the trivial torsor maps to $0$. So an element of
$H^1_{\mathrm{\acute et}}(X,G)$ is the same as an isomorphism class of
$G$-torsors. The Kummer, Artin--Schreier, and Artin--Schreier--Witt
classifications (\Cref{appendix:common_group_scheme_construction},
\Cref{theorem:asw_theory}) use this fact whenever they write $[T_a]$ for
the class of a torsor in $H^1$.
}
...
for every sheaf $\mathcal{F}$, regarded as an $\mathcal{H}$-sheaf with the
trivial action, composition with
$\mathcal{P}\to\mathcal{P}/\mathcal{H}$ identifies morphisms
$\mathcal{P}/\mathcal{H}\to\mathcal{F}$ with the
$\mathcal{H}$-equivariant morphisms $\mathcal{P}\to\mathcal{F}$, equivalently
with morphisms $f$ satisfying $f(hp)=f(p)$ on local sections. A quotient
...
%% [REVIEW: never cited -- admissibility facts are used only implicitly; consider labeling + citing, or compressing into prose with the SGA1 citation]
\begin{prop}
    The following holds:
    \begin{enumerate}
...
Guignard shows that when $X=\Spec(B)$ and $S=\Spec(A)$ then $\Sym_{S}^d(X)$ is representable by an affine $S$-scheme (See \cite[Remark~2.28]{Guignard2018}).
...
\end{theorem}
\begin{proof}
By \stackstag{038K}, a geometrically integral scheme is both geometrically
irreducible and geometrically reduced. Since $X$ is geometrically irreducible,
the projection
\[
X\times_k Y\longrightarrow Y
\]
induces a bijection on irreducible components by \stackstag{038F}.
Because $Y$ is integral, it follows that $X\times_k Y$ is irreducible.
Moreover, $X$ is geometrically reduced and $Y$ is reduced, so
$X\times_k Y$ is reduced by \stackstag{035Z}. Hence it is integral.

If both $X$ and $Y$ are geometrically integral, the same argument applies
after every field extension $k'/k$. Thus
\[
(X\times_kY)_{k'}\cong X_{k'}\times_{k'}Y_{k'}
\]
is integral for every $k'/k$, and therefore $X\times_kY$ is geometrically
integral.
\end{proof}
...
The ring $k[[e_1,\ldots,e_d]]$ is a regular local ring of dimension $d$.
Regularity of a Noetherian local ring is equivalent to regularity of its
completion by \stackstag{07NY}. Hence
$\mathcal{O}_{X^{(d)},z}$ is a regular local ring of dimension $d$.
Since $k$ is algebraically closed and $X^{(d)}$ is locally of finite type
over $k$, regularity at $z$ is equivalent to smoothness at $z$.
...
\]
on $X^{(d)}$. Thus there is a canonical identification
\[
\operatorname{Fr}\bigl(\mathcal{F}^{[d]}\bigr)
\cong \mathcal{P}^{[d]}.
\]
By \Cref{prop:symmetric_power_torsor}, its pullback along
...
\begin{prop}[Semidirect-product description]
\label{prop:symmetric_power_semidirect_quotient}
Let $\Xi_G:=\KG\rtimes S_d$, where $S_d$ acts on $\KG$ by permuting
coordinates. Then $\Xi_G$ acts on $\cP^{\times_S d}$ by
\[
((g_1,\ldots,g_d),\sigma)\cdot(p_1,\ldots,p_d)
=
(g_1p_{\sigma^{-1}(1)},\ldots,g_dp_{\sigma^{-1}(d)}),
\]
...
\]
Moreover, the formation of $\cP^{[d]}$ commutes with flat base change on
$S$. If, in addition, $X$ is flat over $S$, then it commutes with arbitrary
base change; see \cite[Proposition~2.33]{Guignard2018}.
\end{prop}

\begin{proof}
Because $G$ is abelian, the condition $\prod_{i=1}^d g_i=1$ defining
$\KG$ is invariant under permutation of the coordinates. Thus the
$S_d$-action normalizes $\KG$ and defines the semidirect product $\Xi_G$.
By \Cref{prop:symmetric_power_torsor},
\[
\cP^{[d]}=(\KG\backslash\cP^{\times_S d})/S_d.
\]
The universal properties of the two quotients identify this iterated
quotient canonically with $\Xi_G\backslash\cP^{\times_S d}$. Formation of
the torsor quotient $\KG\backslash\cP^{\times_S d}$ commutes with base
change, and the final admissible $S_d$-quotient commutes with flat base
change by \cite[V.1.9]{sga1}. If $X$ is flat over $S$, compatibility with
arbitrary base change is \cite[Proposition~2.33]{Guignard2018}.
\end{proof}
...
For $d\geq 0$, write
\[
X_S^d:=\underbrace{X\times_S\cdots\times_S X}_{d\text{ factors}},
\qquad X_S^0:=S.
\]
The construction of symmetric powers is compatible with a natural addition
law. For any integers $d_1,d_2\geq 0$, concatenating the factors gives a
canonical isomorphism
\[
c_{d_1,d_2}:X_S^{d_1}\times_S X_S^{d_2}
\xrightarrow{\sim}X_S^{d_1+d_2}.
\]
It is equivariant for the inclusion
$S_{d_1}\times S_{d_2}\hookrightarrow S_{d_1+d_2}$ and therefore induces
the canonical \textbf{addition morphism}
\[
+_{d_1,d_2}:X^{(d_1)}\times_S X^{(d_2)}
\longrightarrow X^{(d_1+d_2)}.
\]
...
\[
\begin{tikzcd}
X_S^{d_1}\times_S X_S^{d_2} \arrow[r, "c_{d_1,d_2}", "\sim"'] \arrow[d, "\pi"'] & X_S^{d_1+d_2} \arrow[d, "r_{d_1+d_2}"] \\
