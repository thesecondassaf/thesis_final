We do a reduction theorem \Cref{theorem:SymmetricPowerOfSheafIsTamelyRamified} to the case 
$\mdls=dP$. 
...
\begin{prop}\label{prop:preservationOfTameness} Denote $\mdls_1=n_1P_1$ and $\mdls_2=n_2P_2 + ... + n_rP_r$. Let $X_{\mdls_1}, X_{\mdls_2}$ be the blowups of $C^{(\deg \mdls_1)}, C^{(\deg \mdls_2)}$ by $\mdls_1$, $\mdls_2$ respectively.  
...
\begin{prop}
    In the notations of the previous proposition, suppose $\cF^{(\deg \mdls)}$ is tamely ramified at $\eta$.
...
\textcolor{red}{
\begin{enumerate}
    \item The first proposition copied from takeuchi, should we explain its proof? give another proof? exclude its proof and refer?
    \item Which of the above definition of $C$ are we going to use? (over $k$ or $s$)
    \item In the second proposition, add/explain why are the exceptioanl divisors are irreudcible of codmin 1
    \item Say what is E - the exceiptional divisor of the blowup.
\end{enumerate}
}


\subsubsection{\texorpdfstring{Proof of \Cref{prop:preservationOfTameness}}{Proof of Proposition 25}}
1. s
