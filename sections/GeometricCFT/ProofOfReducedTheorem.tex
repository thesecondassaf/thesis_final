...
By \Cref{theorem:SymmetricPowerOfSheafIsTamelyRamified}, $\cF^{(d)}$ is tamely ramified on the boundary divisor 
$H = \tilde{C}^{(d)}_\mdls \setminus U^{(d)}$.
Thus, by \textcolor{red}{Add reference to this lemma and state it well} \underline{$\cF^{(d)}$ extends to a sheaf $\tilde{\cF}^{(d)}$ on $\tilde{C}^{(d)}_\mdls$ which is unramified along $H$.}
