...
In this section, we establish the following result concerning the stability of tame ramification under the external product of torsors.
...
If $\cG_X$ is tamely ramified at $\eta_X$ and $\cG_Y$ is tamely ramified at $\eta_Y$, then the external product torsor
\[
\cG_{X \times Y} := \text{pr}_1^{-1} \cG_X \otimes \text{pr}_2^{-1} \cG_Y
\]
is tamely ramified at the generic point $\eta_{X\times Y}$ of the exceptional divisor in the product blowup.
\end{prop}
...
\Cref{lemma:weakly_unramified_base_change} shows that the pullbacks of
$\cG_X$ and $\cG_Y$ to $\operatorname{Frac}(S)$ are tamely ramified.
Their contracted product is the restriction of $\cG_{X\times Y}$ to
$\operatorname{Frac}(S)$. Therefore
...
{\color{red}
We first reduce to the case where $k$ is algebraically closed. The formations
of symmetric powers, Takeuchi's blowup and its boundary, and the frame torsor
commute with the flat base change $\Spec(\bar{k})\to\Spec(k)$. At the generic
points of the boundary, this base change induces a weakly unramified extension
of discrete valuation rings with separable residue-field extension. Hence
\Cref{lemma:weakly_unramified_base_change} shows that tameness over $k$ is
equivalent to tameness after base change to $\bar{k}$. We may therefore replace
$k$ by $\bar{k}$.

Because the original field is perfect, each closed point $P_i$ splits into
distinct $\bar{k}$-points, and the base-changed modulus is
\[
   \mdls_{\bar{k}}
      =\sum_i\ \sum_{\bar P\mapsto P_i} n_i\bar P.
\]
In the remainder of the proof we write $k$, $C$, $\mdls$, and $\cF$ for these
base-changed objects.
}
